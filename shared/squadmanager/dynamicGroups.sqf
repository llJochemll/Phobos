...
switch (_mode) do
{
	...
	case "Initialize" :
	{
		...
		if (["IsInitialized"] call Phobos_squadmanagerDynamicGroups) exitWith
		{
		...
		{
			["OnClientMessage", _this] call Phobos_squadmanagerDynamicGroups;
		}];
		...
		{
			["RegisterInitialPlayerGroups", []] call Phobos_squadmanagerDynamicGroups;
		};
		...
		missionNamespace setVariable["custom_group_init",false,true];
	...
	case "OnClientMessage" :
	{
		...
		[_inMode, _inParams] call Phobos_squadmanagerDynamicGroups;
	...
	{
		{
			...
			{
				["RegisterGroup", [_x, leader _x]] call Phobos_squadmanagerDynamicGroups;
			};
		}
		forEach (allGroups - (["GetAllGroups"] call Phobos_squadmanagerDynamicGroups));
	};
	...
	case "RegisterGroup" :
	{
		...
		if (!isNull _group) then
		{
			...
			_insignia	= _data param [0, ["LoadRandomInsignia"] call Phobos_squadmanagerDynamicGroups, [""]];
			...
			{
				["OnPlayerGroupChanged", [_x, _group]] call Phobos_squadmanagerDynamicGroups;
	...
	case "UnregisterGroup" :
	{
		...
		if (!isNull _group && {["IsGroupRegistered", [_group]] call Phobos_squadmanagerDynamicGroups}) then
		{
			...
			{
				["DeleteGroup", [_group]] call Phobos_squadmanagerDynamicGroups;
			};
	...
	case "DeleteGroup" :
	{
		...
		{
			["DeleteGroupLocal", [_group]] call Phobos_squadmanagerDynamicGroups;
		}
		else
		{
	...
	/*case "CreateGroupAndRegister" :
	{
		...
		if (!isNull _player) then
		{
			...
			// Register
			["RegisterGroup", [_newGroup, _player]] call Phobos_squadmanagerDynamicGroups;
	...
	case "AddGroupMember" :
	{
		...
		if (!isNull _player && {!isNull _group} && {group _player != _group}) then
		{
			...
			// Trigger event
			["OnPlayerGroupChanged", [_player, _group]] call Phobos_squadmanagerDynamicGroups;

			// Delete old group
			/*if (count _units < 1) then
			{
				["DeleteGroup", [_oldGroup]] call Phobos_squadmanagerDynamicGroups;
			};*/
		};
	};
	...
	case "RemoveGroupMember" :
	{
		...
		if (!isNull _player && {!isNull _group} && {group _player == _group}) then
		{
			...
			// Trigger event
			["OnPlayerGroupChanged", [_player, group _player]] call Phobos_squadmanagerDynamicGroups;
	...
	case "KickPlayer" :
	{
		...
		if (!isNull _group && {!isNull _leader} && {!isNull _player} && {leader group _leader == _leader} && {group _player == _group}) then
		{
			...
			["RemoveGroupMember", [_group, _player]] call Phobos_squadmanagerDynamicGroups;
	...
	case "UnKickPlayer" :
	{
		...
		if (!isNull _group && {!isNull _player} && {["WasPlayerKickedFrom", [_group, _player]] call Phobos_squadmanagerDynamicGroups}) then
		{
			// The current list of group kicks this player has
			private "_kicksOld";
	...
	case "GetAllGroups" :
	{
		...
		/*{
			if (isPlayer leader _x && {count units _x > 0} && {["IsGroupRegistered", [_x]] call Phobos_squadmanagerDynamicGroups}) then
			{
				_groups pushBack _x;
			};
		} forEach allGroups;*/
	...
	case "GetAllGroupsOfSide" :
	{
		...
		} forEach (["GetAllGroups"] call Phobos_squadmanagerDynamicGroups);
	...
	case "GetGroupByName" :
	{
		...
		_groups = ["GetAllGroups"] call Phobos_squadmanagerDynamicGroups;
		_group  = grpNull;
	...
	case "PlayerHasGroup" :
	{
		...
		["IsGroupRegistered", [group _player]] call Phobos_squadmanagerDynamicGroups;
	};
	...
	case "PlayerIsLeader" :
	{
		...
		_player == leader group _player && ["PlayerHasGroup", [_player]] call Phobos_squadmanagerDynamicGroups;
	};
	...
	case "InitializePlayer" :
	{
		...
		["AddKeyEvents"] call Phobos_squadmanagerDynamicGroups;
		...
		missionNamespace setVariable [VAR_PLAYER_DRAW3D, addMissionEventHandler ["EachFrame",
		{
			...
			if (_timeSinceLastUpdate >= 0.5) then
			{
				{
					[]call Phobos_squadmanagerUpdate;
				...
				if (!isNull _display) then
				{
					...
					(_display displayCtrl IDC_DYNAMICGROUPS_BUTTONCREATELEAVEKICK) ctrlRemoveAllEventHandlers "ButtonClick";
					(_display displayCtrl IDC_DYNAMICGROUPS_BUTTONCREATELEAVEKICK) ctrlEnable false;
					(_display displayCtrl IDC_DYNAMICGROUPS_BUTTONJOIN) ctrlRemoveAllEventHandlers "ButtonClick";
					(_display displayCtrl IDC_DYNAMICGROUPS_BUTTONJOIN) ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnJoinButtonClick", _this] call Phobos_squadmanagerDisplay; }; }];
					(_display displayCtrl IDC_DYNAMICGROUPS_LISTBOXGROUPS) ctrlRemoveAllEventHandlers "treeSelChanged";
					(_display displayCtrl IDC_DYNAMICGROUPS_LISTBOXGROUPS) ctrlAddEventHandler ["treeSelChanged", { with uiNamespace do { ['OnGroupsTreeSelChanged', _this] call Phobos_squadmanagerDisplay; }; }];
				};
		...
		if (_registerInitialGroup && {leader group _player == _player} && {!(["IsGroupRegistered", [group _player]] call Phobos_squadmanagerDynamicGroups)}) then
		{
			["SendClientMessage", ["RegisterGroup", [group _player, _player]]] call Phobos_squadmanagerDynamicGroups;
		};
	};
	...
	case "TerminatePlayer" :
	{
		...
		["RemoveKeyEvents"] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnKeyDown" :
	{
		...
		if (_key in actionKeys UI_OPEN_KEY && !_ctrl) then
		{
			...
			["UpdateKeyDown"] call Phobos_squadmanagerDynamicGroups;
			true;
		}
		else
		{
	...
	case "OnKeyUp" :
	{
		...
		if (!_ctrl && {_key in actionKeys UI_OPEN_KEY} && {isNil { uiNamespace getVariable "BIS_dynamicGroups_ignoreInterfaceOpening" }}) then
		{
			...
			else
			{
				...
				{
					(["GetDisplay"] call Phobos_squadmanagerDisplay) closeDisplay IDC_CANCEL;
				};
			};
	...
	case "UpdateKeyDown" :
	{
		...
		if (!isNil { uiNamespace getVariable "BIS_dynamicGroups_keyDownTime" } && {count (["GetPlayerInvites", [player]] call Phobos_squadmanagerDynamicGroups) > 0}) then
		{
			...
			{
				private "_invites";
				_invites = ["GetPlayerInvites", [player]] call Phobos_squadmanagerDynamicGroups;
				...
				if (count _invites > 0) then
				{
					...
					if (count units _group < missionNamespace getVariable [VAR_MAX_UNITS_PER_GROUP, 99]) then
					{
						...
						// Notification
						["LocalShowNotification", ["DynamicGroups_Joined", [groupId _group]]] call Phobos_squadmanagerDynamicGroups;
					...
					{
						// Notification
						["LocalShowNotification", ["DynamicGroups_PlayerJoinFailed", [_group]]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnInvitationReceived" :
	{
		...
		{
			["LocalShowNotification", ["DynamicGroups_InviteReceived", [name _from], _to]] call Phobos_squadmanagerDynamicGroups;
		};
	...
	{
		private "_insignias";
		_insignias = ["LoadInsignias"] call Phobos_squadmanagerDynamicGroups;
	...
	case "GetInsigniaDisplayName" :
	{
		...
		private "_insignia";
		_insignia = ["LoadInsignia", [_class]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnPlayerGroupChanged" :
	{
		...
		if (["IsGroupRegistered", [_newGroup]] call Phobos_squadmanagerDynamicGroups) then
		{
