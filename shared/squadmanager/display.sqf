...
switch _mode do
{
	case "onLoad":
	{
		...
		_groupsListbox ctrlAddEventHandler ["treeSelChanged", { with uiNamespace do { ['OnGroupsTreeSelChanged', _this] call Phobos_squadmanagerDisplay; }; }];
		_playersListbox ctrlAddEventHandler ["LBSelChanged", { with uiNamespace do { ['OnPlayersLbSelChanged', _this] call Phobos_squadmanagerDisplay; }; }];
		_manageListbox ctrlAddEventHandler ["LBSelChanged", { with uiNamespace do { ["OnManageLbSelChanged", _this] call Phobos_squadmanagerDisplay; }; }];

		// Add double click events to list boxes
		_groupsListbox ctrlAddEventHandler ["TreeDblClick", { with uiNamespace do { ['OnTreeDoubleClick', _this] call Phobos_squadmanagerDisplay; }; }];
		_playersListbox ctrlAddEventHandler ["LBDblClick", { with uiNamespace do { ['OnListDoubleClick', _this] call Phobos_squadmanagerDisplay; }; }];
		_manageListbox ctrlAddEventHandler ["LBDblClick", { with uiNamespace do { ["OnListDoubleClick", _this] call Phobos_squadmanagerDisplay; }; }];

		// Add expanded/collapsed events for Tree
		_groupsListbox ctrlAddEventHandler ["TreeExpanded", { with uiNamespace do { ['OnGroupExpanded', _this] call Phobos_squadmanagerDisplay; }; }];
		_groupsListbox ctrlAddEventHandler ["TreeCollapsed", { with uiNamespace do { ['OnGroupCollapsed', _this] call Phobos_squadmanagerDisplay; }; }];

		// Add event handlers to buttons
		//_createLeaveKickButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnCreateLeaveKickButtonClick", _this] call Phobos_squadmanagerDisplay; }; }];
		_promoteDisbandButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnPromoteDisbandButtonClick", _this] call Phobos_squadmanagerDisplay; }; }];
		_joinButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnJoinButtonClick", _this] call Phobos_squadmanagerDisplay; }; }];
		_invitePlayerButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnInvitePlayerButtonClick", _this] call Phobos_squadmanagerDisplay; }; }];
		_closeInterfaceButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["CloseDisplay"] call Phobos_squadmanagerDisplay; }; }];
		_tabGroupsButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnTabGroupsButtonClick", _this] call Phobos_squadmanagerDisplay; }; }];
		_tabPlayersButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnTabPlayersButtonClick", _this] call Phobos_squadmanagerDisplay; }; }];

		// Add event handler to check box, detect when it's state changes
		_checkboxPrivate ctrlAddEventHandler ["CheckedChanged", { with uiNamespace do { ["OnPrivateStateChanged", _this] call Phobos_squadmanagerDisplay; }; }];

		// Add event handlers to group picture/icon
		_iconPicture ctrlAddEventHandler ["MouseEnter", { with uiNamespace do { ["OnGroupIconMouseEnter", _this] call Phobos_squadmanagerDisplay; }; }];
		_iconPicture ctrlAddEventHandler ["MouseExit", { with uiNamespace do { ["OnGroupIconMouseExit", _this] call Phobos_squadmanagerDisplay; }; }];

		// Add event handlers for editing the group name
		_editGroupName ctrlAddEventHandler ["KeyDown", { with uiNamespace do { ["SetGroupName", _this] call Phobos_squadmanagerDisplay; }; }];
		_editGroupName ctrlAddEventHandler ["SetFocus", { with uiNamespace do { ["OnGroupNameSetFocus", _this] call Phobos_squadmanagerDisplay; }; }];
		_editGroupName ctrlAddEventHandler ["KillFocus", { with uiNamespace do { ["OnGroupNameKillFocus", _this] call Phobos_squadmanagerDisplay; }; }];
		...
		["AddKeyEvents", [_display]] call Phobos_squadmanagerDynamicGroups;
		...
		_createLeaveKickButton ctrlEnable false;

		// Initial update
		["Update", [true]] call Phobos_squadmanagerDisplay;
	...
	case "Update" :
	{
		...
		["UpdateGroupsSection", [_initialUpdate]] call Phobos_squadmanagerDisplay;

		// Update manage section
		["UpdateManageSection", [_initialUpdate]] call Phobos_squadmanagerDisplay;
	...
	case "UpdateManageSection" :
	{
		...
		_playerGroupRegistered	= ["IsGroupRegistered", [_playerGroup]] call Phobos_squadmanagerDynamicGroups;
		...
		else
		{
			...
			// Update members
			["FillPlayersListbox", [_listBoxMembers, _groupPlayers, true]] call Phobos_squadmanagerDisplay;
			...
			_fillPlayerSide ctrlSetText (["GetSideFormattedString", [_groupSide]] call Phobos_squadmanagerDisplay);
			...
			if (!_playerIsLeader || {_initialUpdate} || {_playerGroup != _lastPlayerGroup} || {ctrlText _editGroupName == ["ClampString", [localize "STR_A3_RscDisplayDynamicGroups_Hint"]] call Phobos_squadmanagerDisplay}) then
			{
			...
			private _insigniaTexture = if (_insignia != "") then {["GetInsigniaTexture", [_insignia]] call Phobos_squadmanagerDynamicGroups} else {["GetInsigniaTexture", [_groupPicture]] call Phobos_squadmanagerDynamicGroups};
	...
	case "UpdateGroupsSection" :
	{
		...
		_friendlyGroups = ["GetAllGroupsOfSide", [side group player]] call Phobos_squadmanagerDynamicGroups;
		...
		{
			if (isPlayer _x && {side group _x == side group player} && {!(["IsGroupRegistered", [group _x]] call Phobos_squadmanagerDynamicGroups)}) then
			{
		...
		{
			["OnTabGroupsButtonClick"] call Phobos_squadmanagerDisplay;

			["FillPlayersListbox", [_playersListbox, _sidePlayers]] call Phobos_squadmanagerDisplay;
			uiNamespace setVariable [VAR_OLD_PLAYERS, _sidePlayers];

			["FillGroupsListbox", [_friendlyGroups, _unsortedPlayers]] call Phobos_squadmanagerDisplay;
			uiNamespace setVariable [VAR_OLD_GROUPS, [_friendlyGroups, _unsortedPlayers]];
		}
		else
		{
			["OnTabPlayersButtonClick"] call Phobos_squadmanagerDisplay;
	...
	case "FillGroupsListbox" :
	{
		...
		_selectedIndex 	= missionNamespace getVariable ["selected_groupandplayer", [-1, -1]];
		_totalIndex	= -1;
		...
		// Add them to the list box
		{
			...
			_groupPicture 	= _grp getVariable [VAR_GROUP_INSIGNIA, ["LoadRandomInsignia"] call Phobos_squadmanagerDynamicGroups];
			_groupIsPrivate = _grp getVariable [VAR_GROUP_PRIVATE, false];
			_groupLeader 	= leader _grp;
			_groupPlayers 	= units _grp;
			_wasKickedFrom	= ["WasPlayerKickedFrom", [_grp, player]] call Phobos_squadmanagerDynamicGroups;
			_hasInvite		= ["HasInvite", [_grp, player]] call Phobos_squadmanagerDynamicGroups;
			_locked			= (_groupIsPrivate || {_wasKickedFrom}) && {!(player in _groupPlayers)} && {!_hasInvite};
			_maxUnitCount	= _grp getVariable["max_units", 99];
			_isFull			= count units _grp >= _maxUnitCount;
			_autoName		= if (_maxUnitCount < 99) then { format ["%1  (%2/%3)", _groupName, count ((units _grp) select {isPlayer _x}), _grp getVariable["max_units", 99]] } else { _groupName };

			// Selected group?
			/*if ((uiNamespace getVariable [VAR_SELECTED_GROUPORPLAYER, [grpNull, objNull]]) select 0 == _grp) then
			{
			...
			{
				["GetInsigniaTexture", [_groupPicture]] call Phobos_squadmanagerDynamicGroups;
			};
			...
			if (_grp in (uiNamespace getVariable [VAR_COLLAPSED_GROUPS, []])) then { _listbox tvCollapse [_parent]; } else { _listbox tvExpand [_parent]; };

			{
				private ["_groupPlayerName", "_isLeader", "_sameGroup", "_groupPlayerIcon", "_isDead", "_isIncapacitated"];
				_unit = _x select 2;
				if(isNull _unit)then{
					private _child = _listbox tvAdd [[_parent], format["open (%1)",_x select 0]];
					_listbox tvSetData [[_parent, _child], getPlayerUid _unit];
					_listbox tvSort [[_parent, _child], true];
					_listbox tvSetPicture [[_parent, _child], ICON_GENERAL];
					_listbox tvSetPictureColor [[_parent, _child], COLOR_DEFAULT];
				}else{
					_groupPlayerName 	= [_unit] call BIS_fnc_getName;
					/*
					_isLeader			= _unit == _groupLeader;
					_sameGroup			= group player == group _unit;
					_groupPlayerIcon	= ICON_GENERAL;
					_isDead				= !alive _unit;
					_isIncapacitated	= alive _unit && _unit getVariable ["BIS_revive_incapacitated", false];

					private _color = switch (true) do
					{
						case (_isDead) : 			{ COLOR_DEAD };
						case (_isIncapacitated) : 	{ COLOR_INCAPACITATED };
						case (player == _unit) : 		{ COLOR_SELF };
						case (_sameGroup) : 		{ COLOR_GROUP };
						default 					{ COLOR_DEFAULT };
					};

					// Selected player?
					if ((uiNamespace getVariable [VAR_SELECTED_GROUPORPLAYER, [grpNull, objNull]]) select 1 == _unit) then
					{
					...
					private _child = _listbox tvAdd [[_parent], format["%1 (%2)",_groupPlayerName,_x select 0]];
					_listbox tvSetData [[_parent, _child], getPlayerUid _unit];
					_listbox tvSort [[_parent, _child], true];
					_listbox tvSetPicture [[_parent, _child], ["GetPlayerIcon", [_unit]] call DISPLAY];
					_listbox tvSetPictureColor [[_parent, _child], COLOR_DEFAULT];
				};
			}
			forEach ((_x select 4) select {call (_x select 3) || isPlayer (_x select 2)});
		}
		forEach (playerGroups select {call (_x select 5)});
		...
		if (count _unsortedPlayers > 0) then
		{
...

			{
				...
				_unsortedPlayerIcon	= ["GetPlayerIcon", [_x]] call Phobos_squadmanagerDisplay;
				_isDead			= !alive _x;
	...
	case "FillPlayersListbox" :
	{
		...
		if !(_newPlayersList isEqualTo _oldPlayersList) then
		{
			...
			// Fill listbox
			{
				...
				_textureIcon		= ["GetPlayerIcon", [_x]] call Phobos_squadmanagerDisplay;
				_textureRank		= "a3\Ui_f\data\GUI\Cfg\Ranks\general_gs.paa";
				_isGroupLeader		= _x == leader group _x;
				_texture 			= if (_isGroupLeader && ["IsGroupRegistered", [group _x]] call Phobos_squadmanagerDynamicGroups) then { _textureRank } else { _textureIcon };
				...
				private _index = ["LnbGetDataIndex", [_listbox, getPlayerUid _x]] call Phobos_squadmanagerDisplay;
			...
			// Delete leftovers
			{ ["LnbDeleteUnit", [_listbox, _x]] call Phobos_squadmanagerDisplay; } forEach _unitElements;
		};
	};
	...
	case "TreeUpdateElement" :
	{
		...
		if (_data != "") then
		{
			private _index = ["TreeGetDataIndex", [_listbox, _data]] call Phobos_squadmanagerDisplay;
	...
	case "OnGroupsTreeSelChanged" :
	{
		...
		else
		{
			...
			missionNamespace setVariable ["selected_groupandplayer", [_index select 0, _index select 1]];
	...
	case "OnPlayersLbSelChanged" :
	{
		...
		if !(["PlayerHasGroup", [player]] call Phobos_squadmanagerDynamicGroups) exitWith {};
		...
		_hasInvite 	= ["HasInvite", [group player, _clickedPlayer]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnCreateLeaveKickButtonClick" :
	{
		...
		["SetDefaultFocus"] call Phobos_squadmanagerDisplay:
		...
		_hasGroup 	= ["PlayerHasGroup", [player]] call Phobos_squadmanagerDynamicGroups;
		_isLeader 	= _hasGroup && {player == leader group player};
		_isSelfSelected	= _selectedUid == getPlayeruid player;

		switch (true) do
		{
			case (_hasGroup && {_isLeader} && {_isSelfSelected}) : 	{ ["OnLeaveGroupButtonClick"] call Phobos_squadmanagerDisplay; };
			case (_hasGroup && {_isLeader} && {!_isSelfSelected}) : { ["OnKickPlayerButtonClick", [_selectedUid]] call Phobos_squadmanagerDisplay; };
			case (_hasGroup && {!_isLeader}) :			{ ["OnLeaveGroupButtonClick"] call Phobos_squadmanagerDisplay; };
			case default 						{ ["OnCreateGroupButtonClick"] call Phobos_squadmanagerDisplay; };
		};
	};
	...
	case "OnCreateGroupButtonClick" :
	{
		...
		{
			["SendClientMessage", ["RegisterGroup", [group player, player]]] call Phobos_squadmanagerDynamicGroups;
		}
		else
		{
			["SendClientMessage", ["CreateGroupAndRegister", [player]]] call Phobos_squadmanagerDynamicGroups;
		};
	...
	case "OnLeaveGroupButtonClick" :
	{
		...
		["SendClientMessage", ["RemoveGroupMember", [group player, player]]] call Phobos_squadmanagerDynamicGroups;

		// Event
		["OnGroupLeave", [group player, leader group player, player]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnKickPlayerButtonClick" :
	{
		...
		{
			// Request server to make player leave the current group
			["SendClientMessage", ["KickPlayer", [_group, _groupLeader, _kickedPlayer]]] call Phobos_squadmanagerDynamicGroups;

			// Event
			["OnKicked", [_group, _kickedPlayer, _groupLeader]] call Phobos_squadmanagerDynamicGroups;
		};
	...
	case "OnPromoteDisbandButtonClick" :
	{
		...
		{
			["OnDisbandGroupButtonClick"] call Phobos_squadmanagerDisplay;
		}
		else
		{
			["OnPromotePlayerButtonClick", [_selectedUid]] call Phobos_squadmanagerDisplay;
		};
	...
	case "OnDisbandGroupButtonClick" :
	{
		...
		{
			["OnGroupDisbanded", [_group, _x, _groupLeader]] call Phobos_squadmanagerDynamicGroups;
		...
		// Request server to make player leave the current group
		["SendClientMessage", ["UnregisterGroup", [_group]]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnPromotePlayerButtonClick" :
	{
		...
		["SendClientMessage", ["SwitchLeader", [_group, _newLeader]]] call Phobos_squadmanagerDynamicGroups;

		// Event
		["OnPromotedToLeader", [_group, _newLeader, _groupLeader]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnJoinButtonClick" :
	{
		...
		if (!isNull _group) then
		{
			...
			_groupLeader		= leader _group;
			_groupIsPrivate 	= _group getVariable [VAR_GROUP_PRIVATE, false];
			_hasInvite		= ["HasInvite", [_group, player]] call Phobos_squadmanagerDynamicGroups;
			_isFull			= count units _group >= _group getVariable ["max_units", 99];

			if ((!_groupIsPrivate || {_hasInvite}) && {!_isFull}) then
			{
				if !(["PlayerHasGroup", [player]] call Phobos_squadmanagerDynamicGroups) then
				{
					["SendClientMessage", ["AddGroupMember", [_group, player]]] call Phobos_squadmanagerDynamicGroups;
				}
				else
				{
					["SendClientMessage", ["SwitchGroup", [_group, player]]] call Phobos_squadmanagerDynamicGroups;
				};

				if (_hasInvite) then
				{
					["RemoveInvite", [_group, player]] call Phobos_squadmanagerDynamicGroups;
				};

				// Event
				["OnGroupJoin", [_group, _groupLeader, player]] call Phobos_squadmanagerDynamicGroups;
			};
		};*/

		//["OnGroupJoin", [_group, _groupLeader, player]] call Phobos_squadmanagerDynamicGroups;
		[[missionNamespace getVariable ["selected_groupandplayer", [-1, -1]],player],{
			_groupIndexPrev = (_this select 1) getVariable ["group_index", -1];
			_roleIndexPrev = (_this select 1) getVariable ["role_index", -1];
			_groupIndex = (_this select 0) select 0;
			_roleIndex = (_this select 0) select 1;
			_grp = ((playerGroups select _groupIndex) select 2);
			_unit = ((((playerGroups select _groupIndex) select 4) select _roleIndex) select 2);
			...
			/*if(count ((units _grp) select {_x getVariable["role_index", -1] == _roleIndex}) == 1)then{
				deleteVehicle (((units _grp) select {_x getVariable["role_index", -1] == _roleIndex}) select 0);

				["OnGroupJoin", [_grp, leader _grp, (_this select 1)]] call Phobos_squadmanagerDynamicGroups;
		...
		{
			["Update", [false]] call Phobos_squadmanagerDisplay;
	...
	case "OnInvitePlayerButtonClick" :
	{
		...
		// Add invite
		if !(["HasInvite", [_group, _clickedPlayer]] call Phobos_squadmanagerDynamicGroups) then
		{
			["AddInvite", [_group, player, _clickedPlayer]] call Phobos_squadmanagerDynamicGroups;
		};
	...
	case "OnPrivateStateChanged" :
	{
		...
		["SendClientMessage", ["SetPrivateState", [group player, _isChecked]]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnTreeDoubleClick" :
	{
		...
		if (count _index > 1) then
		{
			...
			{
				["OnPlayerDoubleClick", [_clickedPlayer]] call Phobos_squadmanagerDisplay;
			};
		}
		else
		{
			...
			_clickedGroup	= ["GetGroupByName", [_clickedId, side group player]] call Phobos_squadmanagerDynamicGroups;

			// Group was clicked
			if (!isNull _clickedGroup) then
			{
				["OnGroupDoubleClick", [_clickedGroup]] call Phobos_squadmanagerDisplay;
			};
		};
	};
	...
	case "OnGroupDoubleClick" :
	{
		...
		_wasKickedFrom	= ["WasPlayerKickedFrom", [_group, player]] call Phobos_squadmanagerDynamicGroups;
		_hasInvite	= ["HasInvite", [_group, player]] call Phobos_squadmanagerDynamicGroups;
		_isFull		= count units _group >= missionNamespace getVariable [VAR_MAX_UNITS_PER_GROUP, 99];
	...
	case "OnPlayerDoubleClick" :
	{
		...
		if (_clickedPlayer != player) then
		{
			...
			_selfInGroup 	= ["PlayerHasGroup", [player]] call Phobos_squadmanagerDynamicGroups;
			_otherInGroup	= ["PlayerHasGroup", [_clickedPlayer]] call Phobos_squadmanagerDynamicGroups;
			_hasInvite		= ["HasInvite", [group player, _clickedPlayer]] call Phobos_squadmanagerDynamicGroups;
			_wasKicked		= ["WasPlayerKickedFrom", [group _clickedPlayer, player]] call Phobos_squadmanagerDynamicGroups;
			...
			switch (true) do
			{
				...
				{
					["AddInvite", [group player, player, _clickedPlayer]] call Phobos_squadmanagerDynamicGroups;
				};

				// Join other player's group
				case (!_selfInGroup && {_otherInGroup} && {!_wasKicked} && {!_groupIsLocked} && {!_isFull}) :
				{
					["SendClientMessage", ["AddGroupMember", [group _clickedPlayer, player]]] call Phobos_squadmanagerDynamicGroups;

					if (_hasInvite) then
					{
						["RemoveInvite", [group _clickedPlayer, player]] call Phobos_squadmanagerDynamicGroups;
					};
				};
			};
		};
	...
	case "OnGroupNameChanged" :
	{
		...
		_clampedGroupName 		= ["ClampString", [_groupName]] call Phobos_squadmanagerDisplay;
		_groupNameWasClamped	= _groupNameSize > count (toArray _clampedGroupName);

		private ["_groupsOfSide", "_nameTaken"];
		_groupsOfSide 	= ["GetAllGroupsOfSide", [side group player]] call Phobos_squadmanagerDynamicGroups;
		_nameTaken 		= false;
		...
		if (typeName _clampedGroupName == typeName "" && {_clampedGroupName != _oldGroupName} && {_clampedGroupName != ""} && {!_nameTaken}) then
		{
			...
			["SendClientMessage", ["SetName", [group player, _clampedGroupName]]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnGroupExpanded" :
	{
		...
		_group				= ["GetGroupByName", [_data, side group player]] call Phobos_squadmanagerDynamicGroups;
	...
	case "OnGroupNameKillFocus" :
	{
		...
		["OnGroupNameChanged", [_editGroupName]] call Phobos_squadmanagerDisplay;
	...
	case "GetFactionPicture" :
	{
		...
		{
			["GetSideColor", [side group _leader]] call Phobos_squadmanagerDisplay;
		};
	};
